...
flags.DEFINE_integer("labeled_samples", 10000, "Number of labeled samples to use in train and validation set")
flags.DEFINE_float("validation_percent", 0, "Fraction of labelled data to use for validation")
# Hyperparameters
...
flags.DEFINE_string("weight_initializer", "he_uniform", "Weight initialization method (glorot_uniform/glorot_normal/he_normal/he_uniform)")
# Early stopping
flags.DEFINE_string("stopping", "None", "Early stopping method (cwdeepnnk/deepnnk/validation)")
flags.DEFINE_integer("criterion_freq", 1, "Compute stopping criterion every X epochs")
flags.DEFINE_integer("patience", 20, "Number of times to observe worsening generalization estimate before stopping")
# Graph related parameters
flags.DEFINE_integer("knn_param", 25, "Number of initial neighbors for NNK")
flags.DEFINE_string("kernel", "cosine", "Kernel for NNK graph construction (cosine/gaussian)")
flags.DEFINE_float("interpol_queries", 1.0, "Fraction of training set samples to use as queries in the LOO procedure")
...
def main(arg=None):
    ...
    if labeled_samples > x_train.shape[0]:
        raise Exception("Selected number of labeled samples is larger than training set")
    elif labeled_samples < x_train.shape[0]:
        x_train, _, y_train, _ = train_test_split(x_train, y_train, train_size=float(labeled_samples / x_train.shape[0]), random_state=seed_value, shuffle=True, stratify=y_train)
        
    # Split train and validation set
    val_percent = FLAGS.validation_percent
    if val_percent > 0:
        x_train, x_val, y_train, y_val = train_test_split(x_train, y_train, test_size=val_percent, random_state=seed_value, shuffle=True, stratify=y_train)
    ...
    if 'nnk' in criterion and val_percent > 0:
        raise Exception("You are using a validation set, but you don't need it for NNK label interpolation")
    elif criterion == 'validation' and val_percent == 0:
        raise Exception("You need to select a validation set for Validation-based early stopping")
    ...
    input_data = tf.placeholder(tf.float32, shape=[None] + image_shape, name="input_images")
    ...
    with tf.variable_scope("network", reuse=False):
        # Group 0
        W = tf_utils.weight_variable([5, 5, image_shape[2], num_channels], weight_initializer, name="W_conv0")
        b = tf_utils.bias_variable([num_channels], weight_initializer, name="b_conv0")
        activation_dict[0] = tf_utils.dropout_layer((tf.nn.relu(tf_utils.conv2d_basic_valid(input_data, W, b))), rate=dropout_rate, regularize=regularize)
        # Group 1
        W = tf_utils.weight_variable([5, 5, num_channels, num_channels], weight_initializer, name="W_conv1")
        b = tf_utils.bias_variable([num_channels], weight_initializer, name="b_conv1")
        activation_dict[1] = tf_utils.dropout_layer(tf.nn.relu(tf_utils.conv2d_basic_valid(activation_dict[0], W, b)), rate=dropout_rate, regularize=regularize)
        activation_dict[1] = tf_utils.max_pool_2x2(activation_dict[1])
        # Group 2
        W = tf_utils.weight_variable([5, 5, num_channels, num_channels], weight_initializer, name="W_conv2")
        b = tf_utils.bias_variable([num_channels], weight_initializer, name="b_conv2")
        activation_dict[2] = tf_utils.dropout_layer(tf.nn.relu(tf_utils.conv2d_basic_valid(activation_dict[1], W, b)), rate=dropout_rate, regularize=regularize)
        # Group 3
        W_0 = tf_utils.weight_variable([3, 3, num_channels, 1], weight_initializer, name="W_conv3_ch0")
        W_1 = tf_utils.weight_variable([3, 3, num_channels, 1], weight_initializer, name="W_conv3_ch1")
        W_2 = tf_utils.weight_variable([3, 3, num_channels, 1], weight_initializer, name="W_conv3_ch2")
        W_3 = tf_utils.weight_variable([3, 3, num_channels, 1], weight_initializer, name="W_conv3_ch3")
        W_4 = tf_utils.weight_variable([3, 3, num_channels, 1], weight_initializer, name="W_conv3_ch4")
        ...
        activation_dict[3] = tf_utils.dropout_layer(tf.nn.relu(tf_utils.conv2d_basic_valid(activation_dict[2], W_all, b_all)), rate=dropout_rate, regularize=regularize)
    ...
    W_fc1 = tf_utils.weight_variable([net_size, num_classes], weight_initializer, name="W_fc1")
    b_fc1 = tf_utils.bias_variable([num_classes], weight_initializer, name="b_fc1")
    logits = tf.matmul(net_flatten, W_fc1) + b_fc1
    pred = tf.nn.softmax(logits)
    x_entropy = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(labels=labels, logits=logits))
    ...
    if criterion != 'None':
        name_experiment += f"_patience_{FLAGS.patience}_criterion_freq_{FLAGS.criterion_freq}"
    if 'nnk' in criterion:
    ...
    def get_performance(dataset):
        ...
        for itr in range(n_batches):
            ...
            feed_dict = {input_data: dataset.images[start_idx:end_idx], labels: dataset.labels[start_idx:end_idx]}
    ...
    if mode == 'train':
        if os.path.exists(os.path.join(ckpts_folder, 'checkpoint')):
            raise Exception(f"ckpt directory is not empty. Remove previous training history.\nckpt directory: {ckpts_folder}")
        file_csv = os.path.join(model_output_folder, 'training.csv')
        with open(file_csv, 'w+') as f_csv:
            print('Epoch,Train_Loss,Train_Acc,Val_Loss,Val_Acc,Stopping_Metric,Patience,Best_Val,Channel', file=f_csv)
        ...
        for epoch in range(epochs):
            ...
            for itr in range(n_batches):
                batch_images, batch_labels = train_dataset.next_batch(batch_size=batch_size)
                feed_dict = {input_data: batch_images, labels: batch_labels, dropout_rate: rate}
                _, train_loss[itr], acc = sess.run([train_op, loss, accuracy_op], feed_dict=feed_dict)
                train_acc[itr] = np.mean(acc)
                if itr % 50 == 0:
                    print(f"Epoch: {epoch+1},\tItr: {itr}/{n_batches},  \tLoss: {train_loss[itr]:.4f},\tAcc: {train_acc[itr]:.4f}")
                ...
                if criterion == 'cwdeepnnk':
                    for channel in range(0,num_channels):
                        if ch_patience[channel] > 0:
                            ...
                            activations_train = sess.run([vector], feed_dict={input_data: train_dataset.images, labels: train_dataset.labels})
                            activations_train = np.reshape(activations_train, [train_size, d])
                            # NNK LOO label interpolation error
                            ch_error = nnk_loo(activations=activations_train, labels=train_dataset.labels, interpol_queries=interpol_queries, knn_param=knn_param, kernel=kernel)
                            if best_val[channel] <= ch_error:
                                ch_patience[channel] -= 1
                                print(f'NNK LOO error did not improve: {ch_error:.3f} vs. {best_val[channel]:.3f}. Patience {ch_patience[channel]}/{FLAGS.patience}')
                            else:
                                # Reset channel patience
                                print(f'NNK LOO error improved {best_val[channel]:.3f} -> {ch_error:.3f}')
                            ...
                            with open(file_csv, 'a') as f_csv:
                                print(f'{int(epoch+1)},{np.mean(train_loss)},{np.mean(train_acc)},None,None,{ch_error},{ch_patience[channel]},{best_val[channel]},{channel}', file=f_csv)
                    ...
                    activations_train = np.reshape(activations_train, [train_size, d])
                    # NNK LOO label interpolation error
                    deepnnk_error = nnk_loo(activations=activations_train, labels=train_dataset.labels, interpol_queries=interpol_queries, knn_param=knn_param, kernel=kernel)
                    if best_val <= deepnnk_error:
                        patience -= 1
                        print(f'NNK LOO error did not improve: {deepnnk_error:.3f} vs. {best_val:.3f}. Patience {patience}/{FLAGS.patience}')
                        if patience <= 0:
                        ...
                        saver.save(sess, ckpts_folder + f'/{name_ckpt}.ckpt', epoch+1)
                    with open(file_csv, 'a') as f_csv:
                        print(f'{int(epoch+1)},{np.mean(train_loss)},{np.mean(train_acc)},None,None,{deepnnk_error},{patience},{best_val},None', file=f_csv)

                elif criterion == "validation":
                    if best_val <= val_error:
                        patience -= 1
                        print(f'Validation error did not improve: {val_error:.3f} vs. {best_val:.3f}. Patience {patience}/{FLAGS.patience}')
                        if patience <= 0:
                            print('Breaking train loop: out of patience\n')
                            break
                    else:
                        # Reset patience
                        print(f'\Validation error improved {best_val:.3f} -> {val_error:.3f}')
                        best_val = val_error
                        patience = FLAGS.patience
                        name_ckpt = f"bestval_{epoch+1}"
                        saver.save(sess, ckpts_folder + f'/{name_ckpt}.ckpt', epoch+1)
                    with open(file_csv, 'a') as f_csv:
                        print(f'{int(epoch+1)},{np.mean(train_loss)},{np.mean(train_acc)},{val_loss},{val_acc},{val_error},{patience},{best_val},None', file=f_csv)
                
                # No stopping
                elif val_percent > 0:
                    with open(file_csv, 'a') as f_csv:
                        print(f'{int(epoch+1)},{np.mean(train_loss)},{np.mean(train_acc)},{val_loss},{val_acc},None,None,None,None', file=f_csv)
                else: 
                    with open(file_csv, 'a') as f_csv:
                        print(f'{int(epoch+1)},{np.mean(train_loss)},{np.mean(train_acc)},None,None,None,None,None,None', file=f_csv)
