...
def augment_data(images, data_augmentation=False, is_training=False):
    ...
    if data_augmentation:
        return tf.cond(is_training, lambda: tf.map_fn(lambda x: tf.image.random_flip_left_right(x),
...
def get_examples(x_test, y_test, num_examples, num_classes):
    x_examples = np.zeros([(num_classes * num_examples), (x_test.shape[1]), (x_test.shape[2]), (x_test.shape[3])])
...
def get_examples_by_class(x_test, y_test, num_examples, num_classes):
    if num_examples > (x_test.shape[0] / num_classes):
        raise EnvironmentError("The desired number of examples is too large: %d" % num_examples)
    x_examples = np.zeros([(num_classes), (num_examples), (x_test.shape[1]), (x_test.shape[2]), (x_test.shape[3])])
    y_examples = np.zeros((num_classes, num_examples, num_classes))
    ...
    for index in range(x_test.shape[0]):
        ...
        if countup_array[label] < num_examples: # and selected_index[index] == False: # Random case
            x_examples[label, int(countup_array[label]), :, :, :] = x_test[index, :, :, :]
...
def weight_variable(shape, initializer, stddev=0.02, name=None, trainable=True):
    ...
    if name is None:
        ...
        if initializer == 'glorot_uniform':
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.glorot_uniform(), trainable=trainable)
        elif initializer == 'glorot_normal': # recommended for tanh, logistic, softmax
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.glorot_normal(), trainable=trainable)
        elif initializer == 'he_normal': # he recommended for relu
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.he_normal(), trainable=trainable)
        elif initializer == 'he_uniform': # he recommended for relu
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.he_uniform(), trainable=trainable)
        else:
            return -1
...
def bias_variable(shape, initializer, name=None):
    ...
    if name is None:
        ...
        if initializer == 'glorot_uniform':
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.glorot_uniform())
        elif initializer == 'glorot_normal':
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.glorot_normal())
        elif initializer == 'he_normal':
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.he_normal())
        elif initializer == 'he_uniform':
            return tf.get_variable(name, shape=shape, initializer=tf.initializers.he_uniform())
        else:
            return -1
...
def train(loss_val, var_list, learning_rate, use_Adam=True, beta1=0.9, beta2=0.999, momentum=0.9, global_step=None):
    if use_Adam:
...
def residual_block(input_layer, output_channel, first_block=False):
    ...
    with tf.variable_scope('conv1_in_block'):
        if first_block:
            W = weight_variable(name='conv', shape=[3, 3, input_channel, output_channel])
...
def vgg_network_architecture(input_data, scope_name="network", scope_reuse=False, logs_dir=""):
...
def resnet_architecture(input_tensor_batch, n, scope_reuse=False):
    ...
    for i in range(n):
        with tf.variable_scope('conv1_%d' % i, reuse=scope_reuse):
            if i == 0:
                layer_dict[offset] = residual_block(layer_dict[i + offset - 1], 16, first_block=True)
            else:
